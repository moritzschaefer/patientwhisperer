...
**Full atlas (use this for CellWhisperer scoring and gene expression analysis):**
- Path: `/dfs/user/moritzs/cellwhisperer/results/cd19_atlas_v1/cellwhisperer_clip_v1/cellxgene.h5ad`
- 117,842 cells, 36,117 genes, log1p-normalized expression in `.X`
- `.obsm["transcriptome_embeds"]`: precomputed CellWhisperer embeddings (2048-dim)
- B_Product subset (infusion products): ~39,874 cells from 80 patients
- Filter to OR/NR only for analysis: 79 patients (43 OR, 36 NR)
...
- Path: `/dfs/user/moritzs/cellwhisperer/results/cd19_atlas_v1/cellwhisperer_clip_v1/cellxgene.h5ad`
- 117,842 cells, 36,117 genes, 80 patients (all timepoints)
...
#### Loading and filtering (shared setup)

*Non-restrictive example code* — adapt as needed for your analysis.
...
# Load full atlas and filter to B_Product infusion products, OR/NR only
...
`.X` is **log1p-normalized** — use it directly for gene expression.

*Non-restrictive example code:*
...
The h5ad contains precomputed transcriptome embeddings in `obsm["transcriptome_embeds"]`. `score_transcriptomes_vs_texts` accepts the adata directly and uses these embeddings automatically.
...
# Load model (needed for text embedding + logit_scale)
...
# Score cells against text queries (adata with obsm["transcriptome_embeds"] is used directly)
...
**You are running on a SNAP (Stanford) compute node** with GPU access and the CellWhisperer pixi environment. Patient data files are available locally in the patient directory provided in your prompt.
