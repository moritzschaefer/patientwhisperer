"""
Step 3a: Prepare spatial features from CosMx TME data.
...
Runs on Sherlock (needs /oak/ h5ad files).
...
# Pathology CSV with biopsy_site for lymph node filtering
PATHOLOGY_CSV = os.path.join(os.path.dirname(__file__), "..", "..", "metadata", "TME", "deident_annotated_pathology_v2.csv")
# Fallback paths (OneDrive local, Sherlock)
PATHOLOGY_CSV_LOCAL = "/mnt/onedrive-zina/Good-Lab/T-Cell-Data-Warehouse/Metadata/TME/deident_annotated_pathology_v2.csv"
PATHOLOGY_CSV_SHERLOCK = "/home/users/moritzs/patientwhisperer/metadata/TME/deident_annotated_pathology_v2.csv"

# TMA metadata for excluding low-quality TMAs
TMA_CSV = os.path.join(os.path.dirname(__file__), "..", "..", "metadata", "TME", "cart-cohort-tma-metadata_v2.xlsx")
TMA_CSV_LOCAL = "/mnt/onedrive-zina/Good-Lab/T-Cell-Data-Warehouse/Metadata/TME/cart-cohort-tma-metadata_v2.xlsx"
TMA_CSV_SHERLOCK = "/home/users/moritzs/patientwhisperer/metadata/TME/cart-cohort-tma-metadata_v2.xlsx"
...
if __name__ == "__main__":
    ...
    # Column names identified during QC (from metadata.csv inspection)
    ...
    pathology_path = next(
        (p for p in [PATHOLOGY_CSV, PATHOLOGY_CSV_LOCAL, PATHOLOGY_CSV_SHERLOCK] if os.path.exists(p)),
        None,
    )
    if pathology_path is None:
        raise FileNotFoundError("Pathology CSV not found at any known location")
    ...
    tma_path = next(
        (p for p in [TMA_CSV, TMA_CSV_LOCAL, TMA_CSV_SHERLOCK] if os.path.exists(p)),
        None,
    )
    if tma_path is None:
        raise FileNotFoundError("TMA metadata not found at any known location")
    tma_meta = pd.read_excel(engine="calamine", io=tma_path)
    exclude_ids = set(
